...
export const SEARCH_PROVIDER_OPTIONS: readonly SearchProviderEntry[] = [
  ...
  },
  {
    value: "tavily",
    label: "Tavily",
    hint: "Detailed search for AI agents",
  ...
  },
  {
    value: "searxng",
    label: "SearXNG",
    hint: "Free unlimited search (self-hosted)",
    envKeys: [],
...
export function applySearchKey(
  ...
  switch (provider) {
    ...
    case "tavily":
      ...
      break;
    case "searxng":
      search.searxng = { ...search.searxng, apiKey: key };
      if (extras?.baseUrl) {
        search.searxng.baseUrl = extras.baseUrl;
      }
      break;
  }
  return {
...
export async function setupSearch(
  ...
  if (choice === "searxng") {
    const baseUrlInput = await prompter.text({
      ...
      initialValue: config.tools?.web?.search?.searxng?.baseUrl || "http://localhost:8080",
      placeholder: "http://localhost:8080",
    });
  ...
  if (key || baseUrl) {
    const secretInput = key
      ? resolveSearchSecretInput(choice, key, opts?.secretInputMode)
      : existingKey || "";
