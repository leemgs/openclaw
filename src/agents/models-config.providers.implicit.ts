...
import { buildOwlProvider } from "./models-config.providers.static.js";
...
export async function resolveImplicitProviders(
  ...
  const owlApiKeyRef = resolveSecretInputRef({
    value: params.config?.models?.providers?.owl?.apiKey,
    defaults: params.config?.secrets?.defaults,
  }).ref;
  const owlApiKey =
    (owlApiKeyRef && owlApiKeyRef.source === "env" ? env[owlApiKeyRef.id] : undefined) ??
    env.OWL_API_KEY;
