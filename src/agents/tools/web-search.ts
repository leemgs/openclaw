...
// Note: WebSearchSchema is kept for backwards compatibility if referenced elsewhere,
// but the dynamic createWebSearchSchema is preferred for tool definition.
const _WebSearchSchema = Type.Object({
...
function freshnessToPerplexityRecency(freshness: string | undefined): string | undefined {
  ...
  return FRESHNESS_TO_RECENCY[freshness.toLowerCase()];
}
...
function resolveSearchProvider(search?: WebSearchConfig): (typeof SEARCH_PROVIDERS)[number] {
  ...
  if (raw === "") {
    ...
    if (resolveSearXNGBaseUrl(searxngConfig)) {
      logVerbose(
        'web_search: no provider configured, auto-detected "searxng" (local) from configured base URL',
      );
      return "searxng";
    }
    // Perplexity
...
function resolveSearXNGConfig(search?: WebSearchConfig): SearXNGConfig {
  ...
  const searxng = search?.searxng as SearXNGConfig | undefined;
...
> {
  ...
  return withTrustedWebSearchEndpoint(
    {
      url: params.baseUrl
        ? `${params.baseUrl.trim().replace(/\/$/, "")}/search`
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const country =
        ...
        (isKorean &&
        (provider === "brave" || (provider === "perplexity" && supportsStructuredPerplexityFilters))
          ? "KR"
          : undefined);
      if (
        country &&
      ...
      const search_lang =
        readStringParam(params, "search_lang") ||
        (isKorean && provider === "brave" ? "ko" : undefined);
      ...
      const rawFreshness = readStringParam(params, "freshness") || (isKorean ? "pd" : undefined);
      if (
        rawFreshness &&
        provider !== "brave" &&
        provider !== "perplexity" &&
        ...
        return jsonResult({
          ...
          message: `freshness filtering is not supported by the ${provider} provider. Only Brave, Perplexity, Tavily, and SearXNG support freshness.`,
          docs: "https://docs.openclaw.ai/tools/web",
        });
      }
